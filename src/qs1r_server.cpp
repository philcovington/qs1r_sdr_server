...
void QS1R_server ::showStartupMsg() { std::cout << "This is the QS1R Server version: " << VERSION << std::endl; }
...
int QS1R_server ::initQS1RHardware(unsigned int index = 0) {
    ...
    if (ret == 0) {
        ...
        std::cout << "FPGA ID returned: " << std::hex << (fpga_id = usb.readMultibusInt(MB_VERSION_REG)) << std::dec
                  << std::endl;
