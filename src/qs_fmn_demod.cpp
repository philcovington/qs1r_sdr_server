...
//       m_fmn_phaseError(0), m_fmn_ncoFreq(0), m_fmn_ncoHighLimit(0), m_fmn_ncoLowLimit(0), m_fmn_alpha(0),
//       m_fmn_beta(0), m_fmn_freqDcError(0), m_fmn_dc_alpha(0), m_fmn_outgain(0) {}
...
void QsFMNDemodulator::process(qs_vect_cpx &src_dst) {
    for (auto &sample : src_dst) {
        ...
        // Mix the incoming signal with NCO
        Cpx m_fmn_tmp(m_fmn_cos * sample.real() - m_fmn_sin * sample.imag(),
                      m_fmn_cos * sample.imag() + m_fmn_sin * sample.real());

        // Avoid division by zero by checking the magnitude
        if (std::abs(m_fmn_tmp.real()) < 1e-12 && std::abs(m_fmn_tmp.imag()) < 1e-12) {
            m_fmn_tmp = Cpx(1e-12, 0);
        }
        ...
        // Clamp the NCO frequency to the limits
        m_fmn_ncoFreq = std::clamp(m_fmn_ncoFreq, m_fmn_ncoLowLimit, m_fmn_ncoHighLimit);

        // Update NCO phase
        m_fmn_ncoPhase += (m_fmn_ncoFreq + m_fmn_alpha * m_fmn_phaseError);
        m_fmn_ncoPhase = fmod(m_fmn_ncoPhase, TWO_PI);

        // DC error compensation
        m_fmn_freqDcError = (1.0 - m_fmn_dc_alpha) * m_fmn_freqDcError + m_fmn_dc_alpha * m_fmn_ncoFreq;

        // Output the demodulated FM signal
        float demodulated_value = (m_fmn_ncoFreq - m_fmn_freqDcError) * m_fmn_outgain;
