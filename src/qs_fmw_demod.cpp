...
//       m_fmw_phaseError(0), m_fmw_ncoFreq(0), m_fmw_ncoHighLimit(0), m_fmw_ncoLowLimit(0), m_fmw_alpha(0),
//       m_fmw_beta(0), m_fmw_freqDcError(0), m_fmw_dc_alpha(0), m_fmw_outgain(0) {}
...
void QsFMWDemodulator::process(qs_vect_cpx &src_dst) {
    for (auto &sample : src_dst) {
        ...
        Cpx m_fmw_tmp(m_fmw_cos * sample.real() - m_fmw_sin * sample.imag(),
                      m_fmw_cos * sample.imag() + m_fmw_sin * sample.real());

        // Avoid division by zero by checking the magnitude
        if (std::abs(m_fmw_tmp.real()) < 1e-12 && std::abs(m_fmw_tmp.imag()) < 1e-12) {
            m_fmw_tmp = Cpx(1e-12, 0);
        }
        ...
        m_fmw_ncoFreq = std::clamp(m_fmw_ncoFreq, m_fmw_ncoLowLimit, m_fmw_ncoHighLimit);

        // Update NCO phase and ensure it wraps around TWO_PI
        m_fmw_ncoPhase += (m_fmw_ncoFreq + m_fmw_alpha * m_fmw_phaseError);
        m_fmw_ncoPhase = fmod(m_fmw_ncoPhase, TWO_PI);

        // DC error compensation
        m_fmw_freqDcError = (1.0 - m_fmw_dc_alpha) * m_fmw_freqDcError + m_fmw_dc_alpha * m_fmw_ncoFreq;

        // Output the demodulated wideband FM signal
        float demodulated_value = (m_fmw_ncoFreq - m_fmw_freqDcError) * m_fmw_outgain;
