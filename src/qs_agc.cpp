...
// // QsAgc ::QsAgc()
// //     : m_post_processing_rate(0), m_agc_use_hang(false), m_agc_threshold(-90), m_agc_manual_gain(0),
// m_agc_slope(0),
...
// //       m_agc_attack_avg(0), m_agc_magbuffer_pos(0), m_agc_current_gain(0), m_agc_fixed_manual_gain(0),
// m_agc_knee(0),
...
// //                     else
// //                         m_agc_decay_avg =
// //                             (1.0 - m_agc_decay_fall_alpha) * m_agc_decay_avg + m_agc_decay_fall_alpha *
// m_agc_peak;
// //                 }
...
//     return value > avg ? (1.0 - rise_alpha) * avg + rise_alpha * value : (1.0 - fall_alpha) * avg + fall_alpha *
//     value;
// }
...
//             if (m_agc_use_hang) {
//                 m_agc_attack_avg = update_avg(m_agc_attack_avg, mag, m_agc_attack_rise_alpha,
//                 m_agc_attack_fall_alpha); if (mag > m_agc_decay_avg) {
//                     m_agc_decay_avg = update_avg(m_agc_decay_avg, mag, m_agc_decay_rise_alpha,
//                     m_agc_decay_fall_alpha); m_agc_hang_timer = 0;
...
//             } else {
//                 m_agc_attack_avg = update_avg(m_agc_attack_avg, mag, m_agc_attack_rise_alpha,
//                 m_agc_attack_fall_alpha); m_agc_decay_avg = update_avg(m_agc_decay_avg, mag, m_agc_decay_rise_alpha,
//                 m_agc_decay_fall_alpha);
//             }

//             mag = max(m_agc_attack_avg, m_agc_decay_avg);
//             m_agc_current_gain =
//                 mag <= m_agc_knee ? m_agc_fixed_gain : AGC_OUTPUT_SCALING * pow(10.0, mag * (m_agc_gain_slope
//                 - 1.0));
...
QsAgc::QsAgc()
    : m_processing_rate(50000), m_agc_use_hang(false), m_agc_threshold(-90), m_agc_manual_gain(0), m_agc_slope(0),
      m_agc_hang_time(0), m_agc_hang_time_set(0), m_agc_decay(QS_DEFAULT_AGC_LONG_DECAY), m_agc_decay_set(0),
      m_agc_sample_rate(0), m_agc_sigdly_ptr(0), m_agc_hang_timer(0), m_agc_peak(0), m_agc_decay_avg(0),
      m_agc_attack_avg(0), m_agc_magbuffer_pos(0), m_agc_current_gain(0), m_agc_fixed_manual_gain(0), m_agc_knee(0),
      m_agc_gain_slope(0), m_agc_fixed_gain(0), m_agc_attack_rise_alpha(0), m_agc_attack_fall_alpha(0),
...
void QsAgc::init() {
    auto &global_memory = QsGlobal::g_memory;
    m_agc_decay = global_memory->getAgcDecaySpeed();
    m_processing_rate = global_memory->getDataProcRate();
    m_agc_use_hang = global_memory->getAgcHangTimeSwitch();
    m_agc_threshold = global_memory->getAgcThreshold();
    m_agc_manual_gain = global_memory->getAgcFixedGain();
    m_agc_slope = global_memory->getAgcSlope();
    m_agc_hang_time = global_memory->getAgcHangTime();
    m_agc_hang_time_set = m_processing_rate * m_agc_hang_time * 0.001;

    m_agc_decay_set = m_agc_use_hang ? (m_agc_decay + m_agc_hang_time) : m_agc_decay;

    if (m_agc_sample_rate != m_processing_rate) {
        m_agc_sample_rate = m_processing_rate;
        m_agc_signalDelayBuffer.assign(AGC_MAX_BUFFER_SZ, cpx_zero);
        m_agc_magBuffer.assign(AGC_MAX_BUFFER_SZ, 0.0);

        m_agc_sigdly_ptr = m_agc_hang_timer = m_agc_magbuffer_pos = 0;
        m_agc_peak = m_agc_decay_avg = m_agc_attack_avg = m_agc_current_gain = 0.0;
    }
    ...
    const auto rate_factor = 1.0 / m_processing_rate;
    m_agc_attack_rise_alpha = 1.0 - exp(-rate_factor / AGC_ATTACK_RISE_TC);
    m_agc_attack_fall_alpha = 1.0 - exp(-rate_factor / AGC_ATTACK_FALL_TC);
    m_agc_decay_rise_alpha = 1.0 - exp(-rate_factor / (m_agc_decay_set * 0.001 * AGC_RISEFALL_RATIO));
    m_agc_decay_fall_alpha = 1.0 - exp(-rate_factor / (m_agc_use_hang ? AGC_RELEASE_TC : (m_agc_decay_set * 0.001)));

    m_agc_delay_samples = std::min(static_cast<int>(m_processing_rate * AGC_DELAY_TC), AGC_MAX_BUFFER_SZ - 1);
    m_agc_window_samples = m_processing_rate * AGC_WINDOW_TC;
}
...
void QsAgc::process(qs_vect_cpx &src_dst) {
    ...
    if (m_agc_decay != global_memory->getAgcDecaySpeed()) {
        m_agc_decay = global_memory->getAgcDecaySpeed();
        m_agc_decay_set = m_agc_use_hang ? (m_agc_decay + m_agc_hang_time) : m_agc_decay;
        m_agc_decay_rise_alpha =
            1.0 - exp(-1.0 / (m_processing_rate * m_agc_decay_set * 0.001 * AGC_RISEFALL_RATIO));
        m_agc_decay_fall_alpha =
            1.0 - exp(-1.0 / (m_processing_rate * (m_agc_use_hang ? AGC_RELEASE_TC : m_agc_decay_set * 0.001)));
    }

    if (m_agc_threshold != global_memory->getAgcThreshold() || m_agc_manual_gain != global_memory->getAgcFixedGain() ||
        m_agc_slope != global_memory->getAgcSlope() || m_agc_hang_time != global_memory->getAgcHangTime()) {
        init();
    }
    ...
    if (m_agc_decay == 0) {
        ...
        for (auto &sample : src_dst) {
            delayed_input = m_agc_signalDelayBuffer[m_agc_sigdly_ptr];
            m_agc_signalDelayBuffer[m_agc_sigdly_ptr++] = sample;
            if (m_agc_sigdly_ptr >= m_agc_delay_samples)
                m_agc_sigdly_ptr = 0;

            float mag = max(fabs(sample.real()), fabs(sample.imag()));
            mag = log10(mag + AGC_MIN_CONSTANT);

            float tmp = m_agc_magBuffer[m_agc_magbuffer_pos];
            m_agc_magBuffer[m_agc_magbuffer_pos++] = mag;
            if (m_agc_magbuffer_pos >= m_agc_window_samples)
                m_agc_magbuffer_pos = 0;

            m_agc_peak = (mag > m_agc_peak) ? mag
                         : (tmp == m_agc_peak)
                             ? *max_element(m_agc_magBuffer.begin(), m_agc_magBuffer.begin() + m_agc_window_samples)
                             : m_agc_peak;

            auto update_avg = [&](float &avg, float rise_alpha, float fall_alpha) {
                avg = (m_agc_peak > avg) ? ((1.0 - rise_alpha) * avg + rise_alpha * m_agc_peak)
                                         : ((1.0 - fall_alpha) * avg + fall_alpha * m_agc_peak);
            };

            update_avg(m_agc_attack_avg, m_agc_attack_rise_alpha, m_agc_attack_fall_alpha);
            ...
            if (m_agc_use_hang) {
                ...
                update_avg(m_agc_decay_avg, m_agc_decay_rise_alpha, m_agc_decay_fall_alpha);
            }
        ...
        double gain = 20.0 * log10(m_agc_current_gain) + 3.0;
        global_memory->setAgcCurrentGain(gain);
        global_memory->setAgcCurrentGainC(std::round(gain));
    }
}
