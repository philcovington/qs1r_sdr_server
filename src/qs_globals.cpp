...
std::unique_ptr<QsAudio> QsGlobal::g_audio = std::make_unique<QsAudio>();
std::unique_ptr<QsCircularBuffer<float>> QsGlobal::g_float_rt_ring = std::make_unique<QsCircularBuffer<float>>();
std::unique_ptr<QsCircularBuffer<float>> QsGlobal::g_float_dac_ring = nullptr;
std::unique_ptr<QsCircularBuffer<std::complex<float>>> g_cpx_readin_ring = nullptr;
